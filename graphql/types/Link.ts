...
export const Edge = objectType({
  name: 'Edge',
  definition(t) {
    t.string('cursor'); // returns the last item's cursor
    t.field('node', {
      type: Link
    })
  }
...
export const Response = objectType({
  ...
  definition(t) {
    ...
    t.list.field('edges', { // array of edges (links)
      type: Edge,
    })
  }
})
// client sends a request, can either be the first (no cursor) or...
// ...if next, send another request with a cursor so we know where to jump  
